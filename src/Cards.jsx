...
{item.date}
   ...
   <button className='btn btn-primary'onClick={async()=>{
     try {
        const res = await axios.delete(`http://localhost:8080/data/${item._id}`)
        console.log('Item successfully deleted.')
      } catch (error) {
        alert(error)
      }
...
export function EditForms({cardData}) {
  ...
  const EditCard = async() => {
    ...
    try {
      const res = await axios.put(`http://localhost:8080/data/${cardData._id}`, body)
